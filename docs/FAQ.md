...
TL;DR:
    ...
    RAG asks: Which text is semantically similar?
    HSRAG first asks: Where is this query allowed to retrieve from?
    Then retrieval happens only inside that bounded, auditable space.
...
Minimal example:
    ...
    input/legal_texts/my_law.txt

Example manifest fields:

    {
      "corpus_id": "MY_LAW",
      "title": "My Example Public Law",
      "jurisdiction": "US",
      "source_type": "PUBLIC_LEGAL_TEXT",
      "language": "en",
      "license_note": "User confirms this text is legally usable for local testing.",
      "cthc_domain": "LEGAL",
      "cthc_topic": "GENERAL",
      "text_files": ["my_law.txt"]
    }
...
- text files inside examples/hsrag_law/custom_template/input/legal_texts/
- manifest metadata inside examples/hsrag_law/custom_template/input/manifest.example.json
...
These files are written to:
    ...
    examples/hsrag_law/custom_template/output/
...
The author is a native Chinese speaker, but English discussion is also welcome and will be taken seriously.
...
- text files 放在 examples/hsrag_law/custom_template/input/legal_texts/
- manifest metadata 放在 examples/hsrag_law/custom_template/input/manifest.example.json
