...
HYBRID_MODES = {"HYBRID_BM25_VECTOR", "CTHC_PRUNED_HYBRID"}
...
def main() -> None:
    ...
    parser.add_argument("--config", default="examples/hsrag_law/rq7_scale/config.rq7.hybrid.json")
    ...
    rows = read_csv_rows(Path(runner["metrics_summary"]))
    hybrid_rows = [row for row in rows if row.get("mode") in HYBRID_MODES]
    found_modes = sorted({row["mode"] for row in hybrid_rows})

    passed = runner.get("status") == "OK" and HYBRID_MODES.issubset(set(found_modes))

    output_dir.mkdir(parents=True, exist_ok=True)

    report_md = output_dir / "RQ7_HYBRID_BASELINE_REPORT.md"
    report_json = output_dir / "RQ7_HYBRID_BASELINE_REPORT_SUMMARY.json"
    report_csv = output_dir / "RQ7_HYBRID_BASELINE_REPORT.csv"
    ...
    summary = {
        "schema": "HSRAG_RQ7_HYBRID_BASELINE_REPORT_V0_1",
        ...
        "hybrid_modes": found_modes,
        "hybrid_metric_row_count": len(hybrid_rows),
        "claim_boundary": {
    ...
    lines.extend([
        ...
        "## Metrics",
        "",
        table_row(["mode", "corpus_size", "target_correct", "candidate_reduction", "estimated_p99_ms", "actual_elapsed_p99_ms", "token_cost_per_1k", "esi", "salt_valid"]),
        table_row(["---", "---:", "---:", "---:", "---:", "---:", "---:", "---:", "---:"]),
    ])
